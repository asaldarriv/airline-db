...
{
    public partial class TicketStatusForm : Form
    {
        private readonly TicketStatusFacade _facade = new();
        ...
        {
            var list = _facade.GetAllTicketStatuses().ToList();
            dataGridViewStatuses.DataSource = null;
            dataGridViewStatuses.DataSource = list;
            dataGridViewStatuses.Columns["TicketStatusId"].HeaderText = "ID";
        ...
        private void buttonSave_Click(object sender, EventArgs e)
        {
            ...
            if (editingId == null)
            {
                var newStatus = new TicketStatus
                {
                    StatusCode = code,
                    StatusName = name,
                ...
                _facade.AddTicketStatus(newStatus);
            }
            else
            {
                var updatedStatus = new TicketStatus
                {
                    TicketStatusId = editingId.Value,
                    StatusCode = code,
                    StatusName = name,
                    StatusDescription = description,
                    IsActive = isActive
                };
                _facade.UpdateTicketStatus(updatedStatus);
            }
        ...
        {
            if (dataGridViewStatuses.CurrentRow?.DataBoundItem is TicketStatus selected)
            {
                ...
                {
                    _facade.DeleteTicketStatus(selected.TicketStatusId);
                    LoadStatuses();
                }
            }
            else
        ...
        private void dataGridViewStatuses_SelectionChanged(object sender, EventArgs e)
        {
            if (dataGridViewStatuses.CurrentRow?.DataBoundItem is TicketStatus selected)
            {
                ...
                editingId = selected.TicketStatusId;
                SetFormEnabled(true);
