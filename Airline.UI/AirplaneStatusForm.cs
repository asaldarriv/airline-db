...
{
    public partial class AirplaneStatusForm : Form
    {
        private readonly AirplaneStatusFacade _facade = new();
        private int? editingId = null;

        public AirplaneStatusForm()
        {
            InitializeComponent();
            LoadStatuses();
        ...
        {
            var list = _facade.GetAllAirplaneStatuses().ToList();
            dataGridViewStatuses.DataSource = null;
            dataGridViewStatuses.DataSource = list;
            dataGridViewStatuses.Columns["AirplaneStatusId"].HeaderText = "ID";
        ...
        private void buttonSave_Click(object sender, EventArgs e)
        {
            ...
            if (editingId == null)
            {
                var newStatus = new AirplaneStatus
                {
                    StatusCode = code,
                    StatusName = name,
                ...
                _facade.AddAirplaneStatus(newStatus);
            }
            else
            {
                var updatedStatus = new AirplaneStatus
                {
                    AirplaneStatusId = editingId.Value,
                    StatusCode = code,
                    StatusName = name,
                    StatusDescription = description,
                    IsActive = isActive
                };
                _facade.UpdateAirplaneStatus(updatedStatus);
            }
        ...
        {
            if (dataGridViewStatuses.CurrentRow?.DataBoundItem is AirplaneStatus selected)
            {
                ...
                {
                    _facade.DeleteAirplaneStatus(selected.AirplaneStatusId);
                    LoadStatuses();
                }
            }
            else
        ...
        private void dataGridViewStatuses_SelectionChanged(object sender, EventArgs e)
        {
            if (dataGridViewStatuses.CurrentRow?.DataBoundItem is AirplaneStatus selected)
            {
                ...
                editingId = selected.AirplaneStatusId;
                SetFormEnabled(true);
