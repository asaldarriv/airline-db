...
{
    public partial class ContractsStatusForm : Form
    {
        private readonly ContractsStatusFacade _facade = new();
        private int? editingId = null;

        public ContractsStatusForm()
        {
            InitializeComponent();
            LoadStatuses();
        ...
        {
            var list = _facade.GetAllContractsStatuses().ToList();
            dataGridViewStatuses.DataSource = null;
            dataGridViewStatuses.DataSource = list;
            dataGridViewStatuses.Columns["ContractStatusId"].HeaderText = "ID";
            dataGridViewStatuses.Columns["TypeCode"].HeaderText = "Code";
            dataGridViewStatuses.Columns["Name"].HeaderText = "Name";
            dataGridViewStatuses.Columns["Description"].HeaderText = "Description";
        ...
        private void buttonSave_Click(object sender, EventArgs e)
        {
            ...
            if (editingId == null)
            {
                var newStatus = new ContractsStatus
                {
                    TypeCode = code,
                    Name = name,
                ...
                _facade.AddContractsStatus(newStatus);
            }
            else
            {
                var updatedStatus = new ContractsStatus
                {
                    ContractStatusId = editingId.Value,
                    TypeCode = code,
                    Name = name,
                    Description = description,
                    IsActive = isActive
                };
                _facade.UpdateContractsStatus(updatedStatus);
            }
        ...
        {
            if (dataGridViewStatuses.CurrentRow?.DataBoundItem is ContractsStatus selected)
            {
                ...
                {
                    _facade.DeleteContractsStatus(selected.ContractStatusId);
                    LoadStatuses();
                }
            }
            else
        ...
        private void dataGridViewStatuses_SelectionChanged(object sender, EventArgs e)
        {
            if (dataGridViewStatuses.CurrentRow?.DataBoundItem is ContractsStatus selected)
            {
                ...
                editingId = selected.ContractStatusId;
                SetFormEnabled(true);
